...
# Bug #1/#2  imidazole/benzimidazole: pKa 6.0/5.5 were BASE pKa values, not
#            N-H acid pKa (14/13). Caused charge −1 instead of 0 at pH 7.4.
# Bug A      Phosphonate double-counting: both OHs received pKa1=2.1; pKa2
#            entry was dead code → charge −2 instead of −1 at pH 7.4.
# Bug B      thiol_aliph pKa 10.5 → 9.8; new thiol_alpha_amino pKa=8.3 for
#            Cys-like thiols (correct S⁻ at pH 7.4).
# Bug C      Missing hydroxamic acid entry (vorinostat, panobinostat class).
# Bug D      phosphate_diester label+pKa wrong (2.1→6.1, renamed).
# Bug F      catechol_OH priority over phenol_ortho_CO corrected.
# Bug G      New amine_alpha_EWG entry (pKa=7.5) for alpha-EWG context.
# Bug H      phenol_diacyl (ambiguous SMARTS, pKa=3.5) removed.
# NEW        sulfonyl_imide_NH pKa=2.0 (saccharin, acesulfame-K).
# NEW        enol_lactone pKa=4.2 (ascorbic acid, enol-lactone class).
# NEW        enol_cyclic_dicarbonyl pKa=5.5 (phenylbutazone, pyrazolidinedione).
# NEW        enol_1_3_dicarbonyl pKa=9.0 (acetylacetone, open-chain beta-diketone).
# NEW        pyrazole_NH pKa=14.0, indole_NH pKa=17.0, aliphatic_imine pKa=5.5.
# ─────────────────────────────────────────────────────────────────────────────
