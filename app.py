...
with tab_basic:
    ...
    if lig_input_mode == "SMILES string":
        ...
        try:
            ...
            # ── Shrink Ketcher on mobile via transform: scale ─────────────
            components.html("""
<script>
(function scaleKetcher() {
    try {
        ...
        wrappers.forEach(function(wrap) {
            ...
            var screenW = window.parent.innerWidth;
            if (screenW >= 768) return;   // desktop — do nothing

            var scale    = screenW / 900;
            var origH    = parseInt(iframe.getAttribute('height')) || 420;
            var scaledH  = Math.ceil(origH * scale);
            ...
            // Collapse dead space left by the scale transform
            wrap.style.height   = scaledH + 'px';
            wrap.style.overflow = 'hidden';
        });
    } catch(e) {}
    setTimeout(scaleKetcher, 200);
    setTimeout(scaleKetcher, 800);
    setTimeout(scaleKetcher, 2000);
})();
</script>
""", height=0)
...
with tab_batch:
    ...
    with col_b1:
        ...
        if b_input_mode == "SMILES list (text)":
            st.text_area("One `SMILES [name]` per line (< 20 ligands)",
        ...
        b_ph = st.number_input("Target pH", 2.0, 12.0, 7.4, 0.1, key="b_ph")
