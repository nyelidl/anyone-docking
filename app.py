...
def _calc_adme_properties(smiles: str) -> dict:
    """Calculate ADME properties using RDKit only (offline, no external API)."""
    try:
        ...
        # ── Part 2: RDKit extended (offline) ─────────────────────────────
        ml_results   = {}
        ml_source    = "RDKit (offline)"
        ml_error     = None
        ml_available = True

        try:
            # ESOL aqueous solubility (Delaney 2004)
            esol = round(0.16 - 0.63*logp - 0.0062*mw + 0.066*rotb - 0.74*arom, 2)
            ml_results["solubility"] = esol
            # hERG structural flag (SMARTS screening — not quantitative)
            _HERG_SMARTS = [
                "[NX3;H0]([CX4])([CX4])[CX4]",
                "[n+]",
                "c1ccc2ccccc2c1",
            ...
            ml_results["herg"] = any(
                (lambda p: p and mol.HasSubstructMatch(p))(Chem.MolFromSmarts(s))
                for s in _HERG_SMARTS
            )
            ml_results["herg_prob"] = None
        except Exception as _ex:
            ml_error = str(_ex)
            ml_available = False

        # ── Rule-based ADME estimates ─────────────────────────────────────
        bbb_pts = (
