#!/usr/bin/env python3"""api.py — FastAPI wrapper for Anyone Can Dock.
...
import csvimport htmlimport jsonimport osimport shutilimport tracebackimport uuidimport zipfilefrom pathlib import Pathfrom typing import Any, Dict, List, Literal, Optional, Tuple

import requestsfrom fastapi import BackgroundTasks, Depends, FastAPI, Header, HTTPException, Queryfrom fastapi.responses import FileResponsefrom pydantic import BaseModel, Field, model_validator

from anyonecandock.core import (check_obabel,fix_sdf_bond_orders,get_vina_binary,load_mols_from_sdf,prepare_ligand,prepare_receptor,run_vina,scan_hetatm_residues,)

try:from anyonecandock.core import (calc_rmsd_heavy,write_single_pose,call_poseview_v1,call_poseview2_ref,draw_interaction_diagram,svg_to_png,)except Exception:calc_rmsd_heavy = Nonewrite_single_pose = Nonecall_poseview_v1 = Nonecall_poseview2_ref = Nonedraw_interaction_diagram = Nonesvg_to_png = None

API_TITLE = "Anyone Can Dock API"API_VERSION = "0.1.2"  # [REV] bumpedBASE_WORKDIR = Path(os.getenv("ACD_API_WORKDIR", "/tmp/anyone_can_dock_api")).resolve()BASE_WORKDIR.mkdir(parents=True, exist_ok=True)

API_KEY = os.getenv("DOCKGPT_API_KEY", "").strip()JOBS: Dict[str, Dict[str, Any]] = {}

app = FastAPI(title=API_TITLE,version=API_VERSION,description="API wrapper for Anyone Can Dock.",)

def require_api_key(x_api_key: Optional[str] = Header(default=None)) -> None:if API_KEY and x_api_key != API_KEY:raise HTTPException(status_code=401, detail="Invalid or missing X-API-Key")

class GridBox(BaseModel):center_x: Optional[float] = Field(default=None)center_y: Optional[float] = Field(default=None)center_z: Optional[float] = Field(default=None)size_x: float = Field(default=20.0, gt=0)size_y: float = Field(default=20.0, gt=0)size_z: float = Field(default=20.0, gt=0)
...
class LigandInput(BaseModel):smiles: strname: str = "ligand"

class DockRequest(BaseModel):pdb_id: Optional[str] = Nonereceptor_pdb_text: Optional[str] = Nonereceptor_name: str = "receptor"
...
class DockSubmitResponse(BaseModel):job_id: strstatus: strmessage: strstatus_url: strdownload_url: str

class ScanRequest(BaseModel):pdb_id: Optional[str] = Nonereceptor_pdb_text: Optional[str] = None
...
def safe_name(name: str, default: str = "item") -> str:cleaned = "".join(c if c.isalnum() or c in ".-" else "_" for c in str(name or ""))return cleaned[:80] or default

def _public_url(path: str) -> str:base = os.getenv("PUBLIC_BASE_URL", "").rstrip("/")if not path.startswith("/"):path = "/" + pathreturn f"{base}{path}" if base else path

def _job_url(job_id: str, endpoint: str) -> str:if endpoint == "download":return _public_url(f"/jobs/{job_id}/download")return _public_url(f"/jobs/{job_id}")

def _status_path(job_id: str) -> Path:return BASE_WORKDIR / job_id / "status.json"

def _write_status_file(job_id: str, payload: Dict[str, Any]) -> None:wdir = BASE_WORKDIR / job_idwdir.mkdir(parents=True, exist_ok=True)payload = dict(payload)payload.setdefault("job_id", job_id)payload.setdefault("download_url", _public_url(f"/jobs/{job_id}/download"))_status_path(job_id).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

def _read_status_file(job_id: str) -> Optional[Dict[str, Any]]:path = _status_path(job_id)if not path.exists():return Nonetry:return json.loads(path.read_text(encoding="utf-8"))except Exception:return None

def _write_receptor_input(req: DockRequest | ScanRequest, wdir: Path) -> str:if req.pdb_id:pdb_id = req.pdb_id.strip().upper()if not pdb_id or len(pdb_id) > 12:raise ValueError("Invalid PDB ID")
...
def _compute_blind_box(raw_pdb: str, padding: float = 4.0) -> Tuple[Tuple[float, float, float], Tuple[float, float, float]]:# [REV] Support mmCIF as well as PDB; _write_receptor_input may fall back to a .cif download.if str(raw_pdb).lower().endswith(".cif"):from prody import parseMMCIF
...
def _csv_from_scores(path: Path, rows: List[Dict[str, Any]]) -> None:fieldnames = ["name","input_smiles","prepared_smiles","charge","status","top_score","num_poses","out_pdbqt","out_sdf","pv_sdf","error",]with path.open("w", newline="", encoding="utf-8") as fh:writer = csv.DictWriter(fh, fieldnames=fieldnames, extrasaction="ignore")writer.writeheader()writer.writerows(rows)

def _make_zip(wdir: Path, zip_path: Path) -> None:# [REV] Never include the zip itself or the transient status.json.skip = {zip_path.resolve()}with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:for p in wdir.rglob("*"):if p.is_file() and p.resolve() not in skip and p.name != "status.json":zf.write(p, arcname=p.relative_to(wdir))

def _looks_like_protonation_valence_error(text: str) -> bool:t = (text or "").lower()triggers = ["explicit valence","greater than permitted","valence for atom","sanitize","kekulize",]return any(x in t for x in triggers)

def _is_redocking_case(lig_name: str, rec: Dict[str, Any], req: DockRequest) -> bool:name = (lig_name or "").strip().upper()if not name:return False
...
def _select_pose_for_interaction(pose_sdf: str,rec: Dict[str, Any],req: DockRequest,lig_name: str,) -> Dict[str, Any]:pose_info: Dict[str, Any] = {"selected_pose_rank": None,"pose_selection_method": "","selected_pose_rmsd": None,"warning": "","selected_pose_sdf": "",}
...
def _generate_2d_interaction(job_id: str,wdir: Path,rec: Dict[str, Any],pose_info: Dict[str, Any],smiles: str,lig_name: str,) -> Dict[str, Any]:out: Dict[str, Any] = {"poseview_available": False,"poseview_svg_url": "","poseview_png_url": "","poseview_svg_file": "","poseview_png_file": "","poseview_error": "","poseview_source": "","two_d_interaction_available": False,"two_d_interaction_svg_url": "","two_d_interaction_png_url": "","two_d_interaction_svg_file": "","two_d_interaction_png_file": "","two_d_interaction_error": "",}
...
def _ultra_compact_from_meta(job_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:receptor = meta.get("receptor", {}) if isinstance(meta, dict) else {}results = []
...
def _run_docking_job(job_id: str, req: DockRequest) -> None:wdir = BASE_WORKDIR / job_idwdir.mkdir(parents=True, exist_ok=True)
...
def _restore_completed_job_from_disk(job_id: str) -> Optional[Dict[str, Any]]:wdir = BASE_WORKDIR / job_idmeta_path = wdir / "metadata.json"
...
def _compact_job_response(job_id: str, job: Dict[str, Any]) -> Dict[str, Any]:status = job.get("status", "unknown")
...
@app.get("/health")def health() -> Dict[str, Any]:ob_ok, ob_msg = check_obabel()vina_path, vina_msg = get_vina_binary()
...
@app.get("/ping")def ping() -> Dict[str, Any]:ob_ok, ob_msg = check_obabel()vina_path, vina_msg = get_vina_binary()
...
_BUILTIN_LIGANDS: Dict[str, Dict[str, str]] = {"gefitinib": {"name": "Gefitinib","smiles": "COc1cc2c(cc1OCCCN1CCOCC1)ncnc2Nc1ccc(F)c(Cl)c1","source": "built-in example ligand list",},"erlotinib": {"name": "Erlotinib","smiles": "C#Cc1cccc(Nc2ncnc3cc(OCCOC)c(OCCOC)cc23)c1","source": "built-in example ligand list",},"osimertinib": {"name": "Osimertinib","smiles": "C=CC(=O)Nc1cc(Nc2nccc(-c3cn(C)c4ccccc34)n2)c(OC)cc1N(C)CCN(C)C","source": "built-in example ligand list",},"afatinib": {"name": "Afatinib","smiles": "CN(C)C/C=C/C(=O)Nc1cc2c(Nc3ccc(F)c(Cl)c3)ncnc2cc1O[C@H]1CCOC1","source": "built-in example ligand list",},}

def _pick_smiles_from_pubchem_props(props: Dict[str, Any]) -> str:for key in ("IsomericSMILES", "CanonicalSMILES", "ConnectivitySMILES", "SMILES"):val = props.get(key)if isinstance(val, str) and val.strip():return val.strip()return ""

@app.get("/compound/smiles", dependencies=[Depends(require_api_key)])def compound_smiles(name: str = Query(...),prefer_builtin: bool = Query(True),) -> Dict[str, Any]:q = (name or "").strip()if not q:raise HTTPException(status_code=400, detail="Compound name is required.")
...
@app.get("/pdb/search", dependencies=[Depends(require_api_key)])def pdb_search(query: str = Query(...),top_n: int = Query(10, ge=1, le=25),) -> Dict[str, Any]:q = (query or "").strip()if not q:raise HTTPException(status_code=400, detail="Search query is required.")
...
@app.post("/scan_hetatm", dependencies=[Depends(require_api_key)])def scan_hetatm(req: ScanRequest) -> Dict[str, Any]:job_id = f"scan_{uuid.uuid4().hex[:10]}"wdir = BASE_WORKDIR / job_idwdir.mkdir(parents=True, exist_ok=True)
...
@app.post("/dock", response_model=DockSubmitResponse, dependencies=[Depends(require_api_key)])def submit_docking(req: DockRequest, background_tasks: BackgroundTasks) -> DockSubmitResponse:job_id = f"dock_{uuid.uuid4().hex[:12]}"
...
@app.get("/jobs/{job_id}", dependencies=[Depends(require_api_key)])def get_job(job_id: str) -> Dict[str, Any]:status_payload = _read_status_file(job_id)if status_payload:return status_payload
...
@app.get("/jobs/{job_id}/summary", dependencies=[Depends(require_api_key)])def get_job_summary(job_id: str) -> Dict[str, Any]:return get_job(job_id)

@app.get("/jobs/{job_id}/files/{filename}", dependencies=[Depends(require_api_key)])def get_job_file(job_id: str, filename: str) -> FileResponse:safe_filename = Path(filename).namefile_path = BASE_WORKDIR / job_id / safe_filename
...
@app.get("/jobs/{job_id}/view/{filename}", dependencies=[Depends(require_api_key)])def view_job_file(job_id: str, filename: str):from fastapi.responses import HTMLResponse
...
@app.get("/jobs/{job_id}/download", dependencies=[Depends(require_api_key)])def download_job(job_id: str) -> FileResponse:job = JOBS.get(job_id) or _restore_completed_job_from_disk(job_id)
...
@app.delete("/jobs/{job_id}", dependencies=[Depends(require_api_key)])def delete_job(job_id: str) -> Dict[str, Any]:job = JOBS.pop(job_id, None)wdir = BASE_WORKDIR / job_id
