...
# ── Cloud Run optimisation helpers ────────────────────────────────────────────

@st.cache_data(show_spinner=False)
def _cached_ligand_2d_png(smiles: str) -> bytes:
    """
    Render a 2D structure PNG from SMILES.
    Cached per SMILES string — not re-rendered on every Streamlit rerun.
    Cloud Run benefit: skips ~100 ms of RDKit work on every slider/tab rerun.
    """
    try:
        ...
        buf = io.BytesIO()
        Draw.MolToImage(m, size=(320, 260)).save(buf, format="PNG")
        return buf.getvalue()
    except Exception:
        return b""


def _write_pose_if_stale(mol, path: str, src_sdf: str) -> None:
    """
    Write a single-pose SDF only when the output is missing or older than the
    source SDF. Avoids redundant writes on every Streamlit rerun.
    Cloud Run benefit: reduces ephemeral-disk I/O, frees CPU for billing.
    """
    try:
        if (os.path.exists(path) and os.path.exists(src_sdf)
                and os.path.getmtime(path) >= os.path.getmtime(src_sdf)):
            return
    except OSError:
        pass
...
_DEFAULTS = dict(
    ...
    # Basic docking  (pose_mols removed — loaded on-demand from output_sdf)
    output_pdbqt=None, output_sdf=None, output_pv_sdf=None, dock_base=None,
    docking_done=False, docking_log="", score_df=None,
    # Basic redocking
    ...
    b_plot_png=None,
    # ── Cloud Run optimisation caches ─────────────────────────────────────────
    # Plot PNG caches (keyed by DataFrame hash — skip matplotlib on reruns)
    _basic_affinity_hash=None, basic_affinity_plot_png=None,
    _b_affinity_hash=None,     b_affinity_plot_png=None,
    # Batch ZIP cache (keyed by output file mtimes — skip re-compression)
    _b_zip_hash=None, _b_zip_bytes=None,
    # SDF mol caches — single-entry LRU, evicted on ligand change
    _b_mols_cache=None, _b_mols_cache_path=None,
    _pv_mols_cache=None, _pv_mols_cache_path=None,
)
            ...
            if st.button("🧬 Generate", key=btn_key + "_gen", type="primary"):
                with st.spinner("Generating docked pose diagram…"):
                    try:
                        ...
                        # Single combined call — parsePDB + _detect_all_interactions
                        # run exactly once instead of twice.
                        _svg, _data = draw_interaction_diagram_and_data(
                            receptor_pdb=_rec,
                ...
                if _has_ref_local:
                    with st.spinner("Generating co-crystal diagram…"):
                        try:
                            ...
                            # Single combined call for co-crystal diagram too.
                            _ref_svg, _ref_data = draw_interaction_diagram_and_data(
                                receptor_pdb=_rec,
...
with tab_basic:
    ...
    if st.session_state.ligand_done:
        ...
        with c2d:
            ...
            # _cached_ligand_2d_png is @st.cache_data keyed by SMILES —
            # does not re-render on every slider/tab/rerun.
            _png2d = _cached_ligand_2d_png(st.session_state.prot_smiles)
            if _png2d:
                _png_to_b64_img(
                    _png2d,
    ...
    if st.button(
        ...
        if not dock["success"]:
            ...
            st.session_state.update({
                ...
                "score_df":      df,
                # pose_mols not stored — loaded on-demand in Step 4 results
                "pv_image_png":  None,
                "pv_image_svg":  None,
                "pv_pose_key":   None,
                ...
                "_basic_affinity_hash": None,   # invalidate cached plot
            })
    ...
    if not st.session_state.docking_done:
        ...
        # Load mols on-demand from disk — avoids holding RDKit mol objects
        # in session_state RAM across all reruns.
        _sdf_path = st.session_state.get("output_sdf", "")
        mols = (
            load_mols_from_sdf(_sdf_path, sanitize=False)
            if _sdf_path and os.path.exists(_sdf_path) else []
        )
        ...
        with cc:
            ...
            if df is not None:
                # Only regenerate when score data actually changes.
                # Cloud Run benefit: skips ~100–200 ms matplotlib work on
                # every slider move, tab switch, or widget interaction.
                _ref_score_plot = (st.session_state.get("confirmed_ref_score")
                                   or st.session_state.get("redock_score"))
                _aff_hash = hash((df.to_csv(index=False), _ref_score_plot,
                                  st.session_state.get("confirmed_ref_name")))
                if (st.session_state.get("_basic_affinity_hash") != _aff_hash
                        or not st.session_state.get("basic_affinity_plot_png")):
                    _cc = _chart_colors()
                    ...
                    if _ref_score_plot is not None:
                        ...
                        ax.axhline(
                            ...
                            linestyle="--", label=f"{_ref_nm}: {_ref_score_plot:.2f}",
                        )
                        ax.legend(
                    ...
                    plt.close(fig)
                    st.session_state["basic_affinity_plot_png"] = _buf.getvalue()
                    st.session_state["_basic_affinity_hash"]    = _aff_hash
                _png_to_b64_img(
                    st.session_state["basic_affinity_plot_png"],
        ...
        if _redock_result and _redock_result.get("out_sdf") and os.path.exists(_redock_result["out_sdf"]):
            ...
            if _rd_mols:
                ...
                with _rd_a_col:
                    ...
                    _write_pose_if_stale(_rd_mols[_rd_pose_i], _sp_rd,
        ...
        if mols:
            ...
            with cdl:
                ...
                _write_pose_if_stale(sel_mol, sp_raw,
            ...
            if pv_sdf_all and os.path.exists(pv_sdf_all):
                ...
                _write_pose_if_stale(
                    pv_mols[pose_idx] if pose_idx < len(pv_mols) else sel_mol,
                    sp_pv, pv_sdf_all,
                )
            else:
                _write_pose_if_stale(sel_mol, sp_pv,
                                     st.session_state.get("output_sdf", sp_pv))
...
with tab_batch:
    ...
    if st.button(
        ...
        st.session_state.update({
            ...
            "b_pv2_ref_svg":         None,
            "b_plot_png":            None,
            # Invalidate all cached derived outputs on new batch run
            "_b_affinity_hash":      None,
            "b_affinity_plot_png":   None,
            "_b_zip_hash":           None,
            "_b_zip_bytes":          None,
            "_b_mols_cache":         None,
            "_b_mols_cache_path":    None,
            "_pv_mols_cache":        None,
            "_pv_mols_cache_path":   None,
        })
    ...
    if not b_batch_done:
        ...
        if browsable:
            ...
            # Single-entry LRU: only reload SDF when the selected ligand changes.
            # Cloud Run benefit: avoids re-parsing SDF on every slider/widget rerun.
            _b_mols_path = st.session_state.get("_b_mols_cache_path")
            if _b_mols_path != sel_res["out_sdf"]:
                st.session_state["_b_mols_cache"]      = load_mols_from_sdf(
                    sel_res["out_sdf"], sanitize=False
                )
                st.session_state["_b_mols_cache_path"] = sel_res["out_sdf"]
            b_mols = st.session_state.get("_b_mols_cache", [])
            if b_mols:
                ...
                with cbd:
                    ...
                    _write_pose_if_stale(b_mols[b_pose_i], sp3,
        ...
        if not plot_df.empty:
            ...
            if _n <= 10:
                ...
                with cp2:
                    ...
                    _b_aff_hash = hash((ok_df.to_csv(index=False), active_ref, c_ref_pose))
                    if (st.session_state.get("_b_affinity_hash") != _b_aff_hash
                            or not st.session_state.get("b_affinity_plot_png")):
                        ...
                        plt.close(fig)
                        st.session_state["b_affinity_plot_png"] = _buf.getvalue()
                        st.session_state["b_plot_png"]          = _buf.getvalue()
                        st.session_state["_b_affinity_hash"]    = _b_aff_hash
                    _png_to_b64_img(
                        st.session_state["b_affinity_plot_png"],
                        style="width:100%;height:auto;border-radius:6px;",
                    )
            else:
                st.markdown("**Top Score per Ligand**")
        ...
        with c_zip:
            # Build a content fingerprint from file modification times.
            # Only re-compress when output files actually change.
            # Cloud Run benefit: avoids reading + compressing all output files
            # on every slider move, selectbox change, or tab click.
            _zip_inputs = []
            _rec_fh = st.session_state.get("b_receptor_fh")
            for _r in ([redock_result] if redock_result else []) + ok_results:
                for _k in ("out_sdf", "pv_sdf", "out_pdbqt"):
                    _p = _r.get(_k, "")
                    if _p and os.path.exists(_p):
                        try:
                            _zip_inputs.append((_p, os.path.getmtime(_p)))
                        except OSError:
                            pass
            if _rec_fh and os.path.exists(_rec_fh):
                try:
                    _zip_inputs.append((_rec_fh, os.path.getmtime(_rec_fh)))
                except OSError:
                    pass
            # Also key on PoseView image presence so ZIP stays current
            _zip_inputs.append(("pv2_png", bool(st.session_state.get("b_pv2_image_png"))))
            _zip_inputs.append(("pv2_svg", bool(st.session_state.get("b_pv2_image_svg"))))
            _zip_hash = hash(tuple(_zip_inputs))

            if (st.session_state.get("_b_zip_hash") != _zip_hash
                    or not st.session_state.get("_b_zip_bytes")):
                zb = io.BytesIO()
                with zipfile.ZipFile(zb, "w", zipfile.ZIP_DEFLATED) as zf:
                    ...
                    if st.session_state.get("b_plot_png"):
                        zf.writestr("plots/batch_score_plot.png",
                                    st.session_state["b_plot_png"])
                ...
                zb.seek(0)
                st.session_state["_b_zip_bytes"] = zb.getvalue()
                st.session_state["_b_zip_hash"]  = _zip_hash

            st.download_button(
                "⬇ Download ALL (.zip)",
                st.session_state["_b_zip_bytes"],
        ...
        if pv_browsable:
            ...
            # Single-entry LRU: only reload SDF when the ligand selection changes.
            # Cloud Run benefit: avoids re-parsing on every pose slider move.
            _pv_mols_path = st.session_state.get("_pv_mols_cache_path")
            if _pv_mols_path != pv_sel_res["out_sdf"]:
                st.session_state["_pv_mols_cache"]      = load_mols_from_sdf(
                    pv_sel_res["out_sdf"], sanitize=False
                )
                st.session_state["_pv_mols_cache_path"] = pv_sel_res["out_sdf"]
            pv_all_mols = st.session_state.get("_pv_mols_cache", [])
            ...
            if pv_all_mols:
                ...
                if pv_all_path and os.path.exists(pv_all_path):
                    ...
                    _write_pose_if_stale(pv_all_mols[pv_pose_i], sp_pv2,
                                         pv_sel_res.get("out_sdf", sp_pv2))
